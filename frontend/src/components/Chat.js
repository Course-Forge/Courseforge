...
// import loadingGif from './cfloadinggif.gif'; // LOADING GIF
...
const Chat = () => {
  ...
  const [isTyping, setIsTyping] = useState(false);
  const [courseSuggestion, setCourseSuggestion] = useState(null);  // New state for course suggestion
  const [loading, setLoading] = useState(true); //_______________________: Added loading state
  const hasInitialized = useRef(false);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        // Simulate an API call or prompt load
        await new Promise((resolve) => setTimeout(resolve, 3000)); // 3 seconds delay

        // Example: Load your data here if needed
        // You can set the data here if you have any to set
      } catch (error) {
        console.error("Error loading data:", error);
      } finally {
        setLoading(false); //_______________________: Stop loading
      }
    };
  ...
  return (
    <div className="chat-container">
      <div className="chat-messages">
        ...
        {/*_______________________: Typing indicator conditional rendering */}
        {isTyping ? (
          <div className="chat-message chat-right typing-indicator">
